...
| **正确度** | DeepSeek-V3 多轮投票 | LLM Judge 对答案的医学正确性进行 3 轮独立判定，取投票均值（PASS=1，PARTIAL=0.5，FAIL=0） |
...
4. **正确度**：独立部署的 DeepSeek-V3 作为评委，对每道题进行 3 轮投票（PASS=1 分，PARTIAL=0.5 分，FAIL=0 分），取 3 轮均值作为单题得分。同一道 PARTIAL 题在不同轮次可能被判为 PASS 或 FAIL，导致单题出现 33%（3 票中仅 1 票判 PASS）或 67%（3 票中 2 票判 PASS）等分数。
---
